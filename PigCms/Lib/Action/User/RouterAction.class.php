...
if (0){				
...
}               //先加rounter_confg 再加 Keyword子表（pid、module为Router_config）                 
				$id=$db->add($data1);
				//
				...
				if (1 || intval($arr['ret'])==0){
					...
					$rt=M('Wxuser')->where($this->tokenWhere)->save(array('routerid'=>'router_cfg_'.$id));//$arr['data']['business_id']
					if ($rt){
                                            $this->success('设置成功');
					}else {
