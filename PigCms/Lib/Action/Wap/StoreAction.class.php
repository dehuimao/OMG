 ...
 */             $info = M('reply_info')->where(array('token'=> $this->_get('token'), 'infotype' => 'Shop'))->find();               
                $pics_arr = array();
                if ($info['picurls1'])  $pics_arr['picurls'][] = $info['picurls1'];
                if ($info['picurls2'])  $pics_arr['picurls'][] = $info['picurls2'];
                if ($info['picurls3'])  $pics_arr['picurls'][] = $info['picurls3'];
                
		$date = M('reply_info')->where($where)->find();
		$this->assign('info', $pics_arr);
		$this->assign('metaTitle', '商品分类');
