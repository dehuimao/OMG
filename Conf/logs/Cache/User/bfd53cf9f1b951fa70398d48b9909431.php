...
<div class="content">
          ...
          <div class="cLineB">
              <h4 class="left">当前位置：微学校 >> 教师管理</h4><a href="<?php echo U('School/assess_add',array('token'=>$token,'type'=>'assess'));?>" class="right btnGreen vm" style="margin-top:-2px">新增教师</a>
              <div class="clr"></div>
          </div>
          <div class="cLineD">
    <form method="post" action="<?php echo U('School/search',array('token'=>$token));?>">
    <input type="hidden" name="searchkey[key]" value="assess">
        姓名:<input size="20" value="" name="searchkey[name]" class="px">&nbsp;&nbsp;&nbsp;入职时间:<input size="20" value="" name="searchkey[time]" class="px" >&nbsp;&nbsp;&nbsp;手机号:<input size="20" value="" name="searchkey[phone]" class="px">&nbsp;&nbsp;&nbsp;电子邮件:<input size="20" value="" name="searchkey[email]" class="px">&nbsp;&nbsp;&nbsp;
...
<!--
              ...
              <TBODY>
                <TR></TR>
<?php if(is_array($teachers)): $i = 0; $__LIST__ = $teachers;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><TR>
                  ...
                  <TD><?php echo ($list['jiontime']); ?></TD>
<!--
                 <TD> </TD>
                 <TD> </TD>
                 <TD> </TD> -->
                 <TD>
<a href="<?php echo U('School/assess_add',array('token'=>$token,'tid'=>$list['tid'],'type'=>'edit'));?>"><font color="red">编辑</font></a>
<a class="btn" href="<?php echo U('School/del_item',array('token'=>$list['token'],'id'=>$list['tid'],'deltype'=>'assess'));?>" onclick="return(confirm('确定要删除吗？'))"  title="删除"><font color="#CCC7C7">删除</font></a>
<a href="<?php echo U('School/assess_course',array('token'=>$token,'tid'=>$list['tid']));?>">添加课程</a>
                 </TD>
