...
<div class="tab">
<ul>
<li class="tabli" id="tab0"><a href="<?php echo U('Hotels/index', array('token' => $token,'cid' => $cid));?>">房间类型管理</a></li>
<li class="tabli" id="tab1"><a href="<?php echo U('Hotels/house', array('token' => $token,'cid' => $cid));?>">房间管理</a></li>
<li class="current tabli" id="tab4"><a href="<?php echo U('Hotels/orders', array('token' => $token,'cid' => $cid));?>">订单管理</a></li>
...
<td><?php echo ($o["housename"]); ?></td>
<td><?php echo ($o["nums"]); ?></td>
<td><?php echo ($o["price"]); ?></td>
<td><?php echo ($o["startdate"]); ?></td>
<td><?php echo ($o["enddate"]); ?></td>
<td><?php if($o["paid"] == 1): ?><span style="color:green">已付款</span><?php else: ?><span style="color:red">未付款</span><?php endif; ?> / 
<?php if($o["status"] == 0): ?><span style="color:red">已预定</span>
<?php elseif($o["status"] == 1): ?><span style="color:green">已入住</span>
<?php elseif($o["status"] == 2): ?><span style="color:red">客户取消订单</span>
<?php elseif($o["status"] == 3): ?><span style="color:red">客户删除订单</span><?php endif; ?></td>
<td class="norightborder"><a href="javascript:void(0);" onclick="showIntroDetail(<?php echo ($o["id"]); ?>)">详细</a> 
<a href="javascript:drop_confirm('您确定要删除吗?', '<?php echo U('Hotels/deleteOrder',array('token'=>$token,'id'=>$o['id']));?>');">删除</a> 
</td>
...
function showIntroDetail(id){
	art.dialog.open('<?php echo U('Hotels/orderInfo',array('token'=>$token,'cid'=>$cid));?>&id='+id,{lock:false,title:'订单详情',width:1000,height:620,yesText:'关闭',background: '#000',opacity: 0.87});
}
...
function change(obj) {
	location.href = "<?php echo U('Hotels/orders',array('token' => $token, 'cid' => $cid));?>&status=" + $(obj).val();
}
</script>
</div>
</div>
