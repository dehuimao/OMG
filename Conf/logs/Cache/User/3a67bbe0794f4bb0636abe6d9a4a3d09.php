...
<div class="content"> 
	<div class="cLineB"> 
		<h4>菜品设置</h4> 
		<a href="<?php echo U('Repast/dish',array('token' => $token,'cid'=>$cid));?>" class="right  btnGreen" style="margin-top:-27px">返回</a> 
	</div> 
...
<div class="tab">
<ul>
<li class="tabli" id="tab0"><a href="<?php echo U('Repast/company_index', array('token' => $token,'cid' => $cid));?>">餐厅管理</a></li>
...
<li class="current tabli" id="tab3"><a href="<?php echo U('Repast/dish', array('token' => $token,'cid' => $cid));?>">菜品管理</a></li>
     ...
     <table class="userinfoArea" style=" margin:0;" border="0" cellspacing="0" cellpadding="0" width="100%"> 
	  ...
	  <tr> 
        ...
        <td>  <select name="catid" style="width:400px; height:26px" onchange="chg_catid(this.value)" >
         <?php if(is_array($Companys)): $i = 0; $__LIST__ = $Companys;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$t): $mod = ($i % 2 );++$i;?><option  value="<?php echo ($t["id"]); ?>" <?php if($tableData['catid'] == $t['id']): ?>selected<?php endif; ?>><?php echo ($t["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
        </select> </td> 
       </tr>
       <tr> 
       ...
       <tr> 
        ...
        <td>
        <input type="text" name="price" id="price" value="<?php echo ($tableData["price"]); ?>" class="px" style="width:100px;" /> 元
        </td> 
       </tr>
       <tr> 
        <th><span class="red">*</span>计量单位：</th> 
        <td>
        <input type="text" name="unit" id="unit" value="<?php echo ($tableData["unit"]); ?>" class="px" style="width:100px;" /> (如：份，例，碟)
        </td> 
       </tr>
       <tr> 
        ...
        <td>
	        <select name="sid">
	        <?php if(is_array($dishSort)): $i = 0; $__LIST__ = $dishSort;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><option value="<?php echo ($row["id"]); ?>"  <?php if($row['id'] == $tableData['sid']): ?>selected<?php endif; ?> ><?php echo ($row["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
	        </select>
        </td> 
       </tr>
       <tr> 
        <th>是否推荐：</th> 
        <td>
        <input type="checkbox" name="ishot" id="ishot" value="1" <?php if($tableData['ishot'] == 1): ?>checked<?php endif; ?> />
        </td> 
       </tr>
       <tr> 
        <th>是否上架：</th> 
        <td>
        <input type="checkbox" name="isopen" id="isopen" value="1" <?php if($tableData['isopen'] == 1): ?>checked<?php endif; ?> />
        </td> 
       </tr>
       <tr> 
        <th>展示图片：</th>
       ...
       <th>&nbsp;</th>
       <td>
       <button type="submit" name="submit" class="btnGreen" id="save">保存</button> &nbsp; <a href="<?php echo U('Repast/dish',array('token' => $token, 'cid' => $cid));?>" class="btnGray vm">取消</a></td> 
       </tr> 
      </tbody> 
     </table> 
     </div>
