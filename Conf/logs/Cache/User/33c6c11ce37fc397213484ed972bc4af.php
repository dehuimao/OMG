...
<td class="norightborder">
    <?php if($actinfo['isopen'] == 0): else: ?><a target="_blank" href="<?php echo U('Wall/screen',array('id'=>$actinfo['id'],'token'=>$_SESSION['token']));?>">大屏幕</a><?php endif; ?> <a href="<?php echo U('Wall/prizeRecords',array('id'=>$actinfo['id'],'token'=>$_SESSION['token']));?>">中奖名单</a> 
    <a href="<?php echo U('Wall/edit',array('id'=>$actinfo['id'],'token'=>$_SESSION['token']));?>" >编辑</a>  <a href="javascript:drop_confirm('您确定要删除吗?', '<?php echo U('Wall/del',array('token'=>$token,'id'=>$actinfo['id']));?>');">删除</a>
