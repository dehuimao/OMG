...
<div class="content">
          ...
          <div class="cLineB">
              <h4 class="left">当前位置：微学校 >> 课程安排</h4>
              <div class="clr"></div>
          </div>
          <div class="cLineD">
...
<?php if(is_array($market)): $i = 0; $__LIST__ = $market;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><TR>
                  ...
                  <TD><?php echo ($list['xq_name']); ?></TD>
                  <TD><?php echo ($list['sd_name']); ?></TD>
                  <TD>
<a href="<?php echo U('School/assess_course',array('token'=>$token,'id'=>$list['id'],'tid'=>$list['tid'],'type'=>'edit'));?>"><font color="red">编辑</font></a> &nbsp;&nbsp;
<a class="btn" href="<?php echo U('School/del_item',array('token'=>$token,'id'=>$list['id'],'deltype'=>'curriculum'));?>" onclick="return(confirm('确定要删除吗？'))"  title="删除"><font color="#CCC7C7">删除</font></a>
                 </TD>
