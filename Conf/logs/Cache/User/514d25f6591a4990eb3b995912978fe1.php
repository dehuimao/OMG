...
<a href="javascript:drop_confirm('您确定要删除吗?', '<?php echo U('Host/order_del',array('id'=>$list['id'],'token'=>$token));?>');">删除</a>
<br />
<?php if($list["order_status"] == 1): ?>不可修改
<?php else: ?>
<button type="submit" style="cursor:pointer" ><font color="red">修改</font></button><?php endif; ?>
            ...
            <a href="<?php echo U('Host/lists',array('token'=>$token,'id'=>$_GET['id']));?>" class="right btnGreen vm">返回</a>
          </div>
           <script>
