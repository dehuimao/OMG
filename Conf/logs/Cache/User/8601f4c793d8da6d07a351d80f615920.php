...
<li class="tabli" id="tab2"><a href="<?php echo U('Repast/detail', array('token' => $token,'cid' => $cid));?>">餐厅预定情况</a></li>
...
<!--tab end-->
<div class="cLine">
<div class="pageNavigator left"> <a href="<?php echo U('Repast/company',array('token' => $token,'cid'=>$cid));?>" title="添加" class="btnGrayS vm bigbtn"><img src="<?php echo RES;?>/images/product/add.png" width="16" class="vm">添加餐厅</a></div>
<div class="clr"></div>
</div>
<div class="msgWrap">
...
<thead>
	<tr>
		...
		<th width="">邮件通知</th>
        <th width="">无线打印</th>
		<th width="" class="norightborder">操作</th>
	</tr>
</thead>
<tbody>
<tr></tr>
<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$tablelist): $mod = ($i % 2 );++$i;?><tr>
		<td><?php echo $Companys_arr[$tablelist['catid']];?></td>
		<td>
        <?php if ($tablelist['istakeaway'] == 0) echo '不支持'; else echo '支持'; ?>
        </td>
		<td><?php echo ($tablelist["price"]); ?></td>
		<td><?php echo ($tablelist["category"]); ?></td>
		<td>
        <?php if ($tablelist['phone_status'] == 0) echo '关闭'; else echo '开启'; ?>
        </td>
		<td>
        <?php if ($tablelist['phone_authorize'] == 0) echo '关闭'; else echo '开启'; ?>
        </td>        
		<td>
        <?php if ($tablelist['email_status'] == 0) echo '关闭'; else echo '开启'; ?>
        </td>
		<td>
        <?php if ($tablelist['print_status'] == 0) echo '关闭'; else echo '开启'; ?>
        </td>        
		<td class="norightborder">
			<a href="<?php echo U('Repast/company', array('id' => $tablelist['id'], 'token' => $token,'cid' => $tablelist['cid']));?>">修改</a> 
			<a href="javascript:drop_confirm('您确定要删除吗?', '<?php echo U('Repast/company_del',array('id' => $tablelist['id'], 'token' => $token,'cid' => $tablelist['cid']));?>');">删除</a>
		</td>
