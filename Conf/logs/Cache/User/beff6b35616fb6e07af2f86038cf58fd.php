...
<div class="content">   
      <div class="cLineB">
        <h4 class="left">    
          投票数据统计
          (<?php if($vote['cknums'] == 1): ?>单选<?php else: ?>多选<?php endif; ?>投票，<span class="number">共有<?php echo ($vote["count"]); ?>人参与投票</span>)
        </h4>   
        <span style="float:right;">
            <a class="btnGreen" href="javascript:window.history.go(-1);">返回</a>
        </span>     
        <div class="clr"></div>
      </div>
      ...
      <div class="msgWrap" style="border:1px solid #e6e6e6;margin-top:10px;">
         ...
         <script type="text/javascript">
          var chart = new FusionCharts("/tpl/static/fushionCharts/Charts/Pie3D.swf", "ChartId", "500", "400", "0", "1");
      ...
      <div class="cLineB">
        ...
        <table class="ListProduct" width="90%" border="0" cellspacing="0" cellpadding="0">
          <thead>   
          <tr>
            <th width="200">投票人</th>
            <th width="200">选票</th>
          ...
          <tbody>
          <?php if(is_array($record)): $i = 0; $__LIST__ = $record;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><tr>
              <td><?php echo ($item["wxname"]); ?></td>
              <td><?php echo ($item["itemname"]); ?></td>
              <td><?php echo (date("Y-m-d H:i:s",$item["touch_time"])); ?></td>
              <td><a href="<?php echo U('Vote/del_record',array('id'=>$item['id']));?>" onclick="javascript:return confirm('您确定要删除吗?')">删除</a></td>
