...
<!--tab end-->
<div class="cLine">
<div class="pageNavigator left"> <a href="<?php echo U('Repast/dishadd',array('token' => $token,'cid'=>$cid));?>" title="添加" class="btnGrayS vm bigbtn"><img src="<?php echo RES;?>/images/product/add.png" width="16" class="vm">添加菜品</a></div>
<div class="clr"></div>
</div>
<div class="msgWrap">
...
<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$tablelist): $mod = ($i % 2 );++$i;?><tr>
		...
		<td class="norightborder">
			<a href="<?php echo U('Repast/dishadd', array('id' => $tablelist['id'], 'token' => $token,'cid' => $tablelist['cid']));?>">修改</a> 
			<a href="javascript:drop_confirm('您确定要删除吗?', '<?php echo U('Repast/dishdel',array('id' => $tablelist['id'], 'token' => $token,'cid' => $cid));?>');">删除</a>
		</td>
