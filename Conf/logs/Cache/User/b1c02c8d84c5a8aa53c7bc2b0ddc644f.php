...
<div class="content">
          <div class="cLineB">
              <h4 class="left">车主管理</h4>
              <div class="clr"></div>
          </div>
           <div class="cLine">
    <div class="alert">
    <p><span class="bold">注意：</span>根据保险费用和里程费用倒序排序，更多信息请查看［费用详细］ </p>
    </div>
</div>
          <div class="cLine">
          ...
          <input name="delall" type="hidden" value="del">
           <input name="wxid" type="hidden" value="gh_423dwjkewad">
            <table class="ListProduct" border="0" cellspacing="0" cellpadding="0" width="100%">
              ...
              <tbody>
              <?php if(is_array($onwers)): $i = 0; $__LIST__ = $onwers;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                    <td><?php echo ($vo['car_userName']); ?></td>
                       <td><?php echo ($vo['brand_serise']); ?></td>
                       <td><?php echo ($vo['carmodel']); ?></td>
                        <td><?php echo ($vo['user_tel']); ?></td>
                       <td><?php echo ($vo['car_no']); ?></td>
                        <td><?php echo ($vo['car_buyTime']); ?></td>
                       <td><?php echo ($vo['car_insurance_lastCost']); ?></td>
                       <td><?php echo ($vo['car_care_lastCost']); ?></td>

                       <td class="input-medium">
                            <a href="###" onclick="showIntroDetail(<?php echo ($vo["id"]); ?>)">费用详细</a>
                            &nbsp;&nbsp;&nbsp;&nbsp;
                            <a class="btn" href="<?php echo U('Car/del_carowner',array('token'=>$token,'id'=>$vo['id']));?>" title="删除" onclick="return(confirm('确定要删除吗？'))" >删除</a></td>
          ...
          <div class="cLine">
            ...
            function showIntroDetail(id){
                art.dialog.open('<?php echo U('Car/owner_uinfo',array('token'=>$token));?>&id='+id,{lock:false,title:'车主详情',width:700,height:520,yesText:'关闭',background: '#000',opacity: 0.87});
            }
        </script>
            <div class="clr"></div>
          </div>
