...
<div class="content">
          <div class="cLineB">
              <h4 class="left">楼盘相册管理</h4>
               
          </div>
          
          <div class="cLine">
    <div class="alert">
    <p><span class="bold">使用方法：</span><?php if($album != ''): ?><span style="color:red;">（注：系统默认为至少添加3个相册,注意不要重复添加同一个相册）</span><span style="color:red;">（注：请不要重复添加同一个相册，否则删除一个同样的相册都删除）修改<strong><a href="<?php echo U('Photo/index',array('token'=>$token));?>"><font color='green'>[相册内容]</font></a></strong></span>
                                <?php else: ?>
                                 <span style="color:red;"> 请您先去［楼盘简介］添加楼盘信息。再来添加楼盘相册</span><?php endif; ?></p>
    </div>
</div>
          
          <div class="cLine">
            <div class="pageNavigator left">
                <a href="<?php echo U('Estate/album_add',array('token'=>$token));?>" class="btnGrayS vm bigbtn">
                    <img src="tpl/User/default/common/images/pic.png" class="vm">添加相册
                </a>
            </div>
            <div class="clr"></div>
          </div>
            ...
            <table class="ListProduct" border="0" cellspacing="0" cellpadding="0" width="100%">
              <thead>
                                        <tr>
                                            ...
                                            <tbody>
                                        <?php if($album == ''): ?><tr>
                                        <td colspan="5" style="text-align:center; height:30px;"><strong>无任何楼盘信息.</strong></td>
                                        </tr>
                                    <?php else: ?>
                                    <?php if(is_array($album)): $i = 0; $__LIST__ = $album;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr> 
                                        <td style="text-align:center;"><?php echo ($list['title']); ?></td>
                                        <td style="text-align:center;"><?php echo ($list['info']); ?></td> 
                                        <td style="text-align:center;">
                                        <a href="<?php echo U('Estate/album_add',array('poid'=>$list['id'],'token'=>$list['token']));?>" class="btn">编辑</a>
                                        <a href="<?php echo U('Estate/album_del',array('id'=>$list['mid'],'token'=>$list['token']));?>" onclick="return(confirm('确定要删除吗？'))" class="btn">删除</a></a>
</td>
