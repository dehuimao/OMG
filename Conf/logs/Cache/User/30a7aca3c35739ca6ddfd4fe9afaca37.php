...
<div class="content">
  ...
  <div class="cLineC" style="line-height: 32px;"> <a href="<?php echo U('Business/comments_add',array('token'=>$token,'type'=>$type));?>" class="btnGrayS vm bigbtn">添加点评</a>　 </div>
...
美容公司<?php endif; ?>
          ...
          <tbody>
              <?php if(is_array($comments)): $i = 0; $__LIST__ = $comments;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                    ...
                    <td>
              <a href="<?php echo U('Business/comments_add',array('cid'=>$vo['cid'],'type'=>$vo['type']));?>" class="btnGreen">编辑</a>
              <a href="<?php echo U('Business/comments_del',array('cid'=>$vo['cid'],'type'=>$vo['type']));?>" onclick="return(confirm('确定要删除吗？'))" class="btnGreen">删除</a>
                    </td>
