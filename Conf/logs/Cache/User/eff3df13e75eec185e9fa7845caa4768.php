...
<div class="msgWrap form">
        ...
        <li <?php if(ACTION_NAME == 'record'): ?>class="selectTag"<?php endif; ?>>
            ...
            <li style="float:right;"><a href="<?php echo U('Custom/index',array('token'=>$token));?>" class="btnGreen">返回</a></li>
...
<div class="cLine">
<div class="left">
<a href="<?php echo U('Custom/exportForms',array('token'=>$token,'set_id'=>$set_id));?>" class="btnGrayS vm" style="height:25px;">
导出用户表单
</a>
</div>
...
<th width="40">用户名称</th>
<th width="30">提交时间</th>
<?php if(is_array($field)): $i = 0; $__LIST__ = $field;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$field): $mod = ($i % 2 );++$i;?><th width="30"><?php echo ($field["field_name"]); ?></th><?php endforeach; endif; else: echo "" ;endif; ?>
...
<td><?php echo (date("Y-m-d",$item["add_time"])); ?></td>
	<?php if(is_array($item['ex_info'])): $i = 0; $__LIST__ = array_slice($item['ex_info'],0,6,true);if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ex_info): $mod = ($i % 2 );++$i;?><td><?php echo ($ex_info["value"]); ?></td><?php endforeach; endif; else: echo "" ;endif; ?>
<td class="norightborder">
	<a href="javascript:void(0);" onclick="showIntroDetail(<?php echo ($item["info_id"]); ?>)">查看详情</a>
	|
	<a href="javascript:drop_confirm('您确定要删除吗?', '<?php echo U('Custom/infoDel',array('info_id'=>$item['info_id'],'token'=>$token));?>');">删除</a>
</td>
...
function showIntroDetail(id){
	art.dialog.open('<?php echo U('Custom/detail',array('token'=>$token));?>&info_id='+id,{lock:false,title:'信息详情',width:400,height:300,yesText:'关闭',background: '#000',opacity: 0.87});
}
</script>
</div>
</div>
