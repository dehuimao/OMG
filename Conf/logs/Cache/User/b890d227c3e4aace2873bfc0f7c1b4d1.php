...
<div class="content">
          <div class="cLineB">
              <h4 class="left">实用工具</h4>
              <div class="clr"></div>
          </div>

          <div class="cLine">
    <div class="alert">
    <p><span class="bold">使用方法：</span>(第一次启用请先自己添加对应的工具,每次只能添加一项)开启状态可以在手机显示，关闭就不显示,您也可以修改URL</p>
    </div>
</div>
...
<?php if(is_array($utility)): $i = 0; $__LIST__ = $utility;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>jQuery("#formID<?php echo ($i); ?>").validationEngine("attach",{
    promptPosition:"centerRight",
    autoPositionUpdate:true
  });<?php endforeach; endif; else: echo "" ;endif; ?>
    });
</script>
                <?php if(is_array($utility)): $i = 0; $__LIST__ = $utility;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><form method="post" action="" id="formID<?php echo ($i); ?>">

                            <tr>
                              <input type="hidden"  name="id" value="<?php echo ($vo['id']); ?>">
                                <td><span class="sn weather"><input type="text" name="name" id="name<?php echo ($i); ?>" value="<?php echo ($vo['name']); ?>" data-validation-engine="validate[required,minSize[2],maxSize[10]]"
                    data-errormessage-value-missing="必填项!"></span></td>
                                <td><span class="sn weather"><input type="text" name='url' style="width:400px;" value="<?php echo ($vo['url']); ?>" id="url<?php echo ($i); ?>" data-validation-engine="validate[required,custom[url]]"
                    data-errormessage-value-missing="必须填写网址!"  data-errormessage-custom-error="请填写正确的网址"></span></td>

                                <td>
                                <select name="status" id="status">
                                <option value="1" <?php if($vo['status'] == 1): ?>selected="selected"<?php endif; ?>>开启</option>
                                  <option value="2" <?php if($vo['status'] == 2): ?>selected="selected"<?php endif; ?>>关闭</option>
                                </select>
                                </td>
                                <td><button type="submit" name="button" id="button<?php echo ($i); ?>" class="btnGreen">保存</button>
<a href="<?php echo U('Car/item_del',array('id'=>$vo['id'],'type'=>'del'));?>" onclick="return(confirm('确定要删除吗？'))" class="btnGreen">删除</a>
                                </td>
                            </tr>

 </form><?php endforeach; endif; else: echo "" ;endif; ?>

<?php if($utility == ''): ?><form method="post" action="" id="formID11">

                            <tr>
                              <input type="hidden"  name="token" value="<?php echo ($token); ?>">
                                <td><span class="sn weather"><input type="text" name="name" id="name11" value="车贷计算器" data-validation-engine="validate[required,minSize[2],maxSize[10]]"
                    data-errormessage-value-missing="必填项!"></span></td>
                                <td><span class="sn weather"><input type="text" name='url' style="width:400px;" value="http://car.m.yiche.com/qichedaikuanjisuanqi" id="url11" data-validation-engine="validate[required,custom[url]]"
                    data-errormessage-value-missing="必须填写网址!"  data-errormessage-custom-error="请填写正确的网址"></span></td>
                                ...
                                <td><button type="submit" name="button" id="button<?php echo ($i); ?>" class="btnGreen">保存</button></td>
                            </tr>
...
<form method="post" action="">
                            ...
                            <tr>
                              <input type="hidden"  name="token" value="<?php echo ($token); ?>">
                                <td><span class="sn weather"><input type="text" name="name" placeholder="工具名称" id="name10" value="" data-validation-engine="validate[required,minSize[2],maxSize[10]]"
                    data-errormessage-value-missing="必填项!"></span></td>
                                <td><span class="sn weather"><input type="text" name='url' style="width:400px;" value="" id="url10" data-validation-engine="validate[required,custom[url]]"
                    data-errormessage-value-missing="必须填写网址!"  data-errormessage-custom-error="请填写正确的网址" placeholder="工具对应的网址"></span></td>
