...
<li class="current tabli" id="tab1"><a href="<?php echo U('Hotels/house', array('token' => $token,'cid' => $cid));?>">房间管理</a></li>
...
<!--tab end-->
<div class="cLine">
<div class="pageNavigator left"> <a href="<?php echo U('Hotels/houseadd',array('token' => $token,'cid'=>$cid));?>" title="添加" class="btnGrayS vm bigbtn"><img src="<?php echo RES;?>/images/product/add.png" width="16" class="vm">添加房间</a></div>
<div class="clr"></div>
</div>
<div class="msgWrap">
...
<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$tablelist): $mod = ($i % 2 );++$i;?><tr>
		...
		<td><?php echo ($tablelist["sname"]); ?></td>
		<td><?php echo ($tablelist["note"]); ?></td>
		<td class="norightborder">
			<a href="<?php echo U('Hotels/houseadd', array('id' => $tablelist['id'], 'token' => $token,'cid' => $tablelist['cid']));?>">修改</a> 
			<a href="javascript:drop_confirm('您确定要删除吗?', '<?php echo U('Hotels/housedel',array('id' => $tablelist['id'], 'token' => $token,'cid' => $tablelist['cid']));?>');">删除</a>
		</td>
