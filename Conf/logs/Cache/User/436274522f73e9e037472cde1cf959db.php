...
<div class="content">
          <div class="cLineB">
              <h4 class="left">楼盘管理</h4>
          </div>
  
          <div class="cLine">
            <div class="pageNavigator left">
                <a href="<?php echo U('Estate/set',array('token'=>$token));?>" class="btnGrayS vm bigbtn">
                    <img src="tpl/User/default/common/images/pic.png" class="vm">添加楼盘
                </a>
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                <a href="<?php echo U('Estate/reservation',array('token'=>$token));?>" class="btnGrayS vm bigbtn">
                    <img src="tpl/User/default/common/images/pic.png" class="vm">预约管理
                </a>
            </div>
            <div class="clr"></div>
          </div>
          ...
          <input name="delall" type="hidden" value="del">
           <input name="wxid" type="hidden" value="gh_423dwjkewad">
            <table class="ListProduct" border="0" cellspacing="0" cellpadding="0" width="100%">
              <thead>
                <tr>
                  <th class="span4">楼盘名称</th>
                  <th class="span4">关键词</th>
                  <th class="span2">操作</th>
                </tr>
              </thead>
              <tbody>
                <?php if($estate == ''): ?><tr>
                    <td colspan="4" style="text-align:center; height:30px;">没有任何楼盘信息</td>
                  </tr>
                <?php else: ?>
                  <?php if(is_array($estate)): $i = 0; $__LIST__ = $estate;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
                      <td style="text-align:center; "><?php echo ($list['title']); ?></td>
                      <td style="text-align:center; ;"><?php echo ($list['keyword']); ?></td>
                      <td style="text-align:center; ">
                        <a href="<?php echo U('Estate/nav',array('pid'=>$list['id'],'token'=>$list['token']));?>" class="btn">楼盘版块</a>
                        |
                        <a href="<?php echo U('Estate/son',array('pid'=>$list['id'],'token'=>$list['token']));?>" class="btn">子楼盘</a>
                        |
                        <a href="<?php echo U('Estate/housetype',array('pid'=>$list['id'],'token'=>$list['token']));?>" class="btn">户型</a>
                        |
                        <a href="<?php echo U('Estate/album',array('pid'=>$list['id'],'token'=>$list['token']));?>" class="btn">相册</a>
                        |
                        <a href="<?php echo U('Estate/impress',array('pid'=>$list['id'],'token'=>$list['token']));?>" class="btn">房友印象</a>
                        |
                        <a href="<?php echo U('Estate/expert',array('pid'=>$list['id'],'token'=>$list['token']));?>" class="btn">专家点评</a>
                        |
                        <a href="<?php echo U('Estate/reservation_manage',array('pid'=>$list['id'],'token'=>$list['token'],'addtype'=>'house_book','id'=>$list['res_id']));?>" class="btn">预约订单</a>
                        |
                        <a href="<?php echo U('Estate/aboutus',array('pid'=>$list['id'],'token'=>$list['token']));?>" class="btn">关于我们</a>
                        |
                        <a href="<?php echo U('Estate/set',array('id'=>$list['id'],'token'=>$list['token']));?>" class="btn">编辑</a>
                        |                
                        <a href="<?php echo U('Estate/del',array('id'=>$list['id'],'token'=>$list['token']));?>" onclick="return(confirm('确定要删除吗？'))" class="btn">删除</a></a>
                      </td>
