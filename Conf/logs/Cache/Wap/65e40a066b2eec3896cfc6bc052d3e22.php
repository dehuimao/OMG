...
<section class="f_list i_list" >
		...
		<?php if(is_array($products)): $i = 0; $__LIST__ = $products;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$hostlist): $mod = ($i % 2 );++$i;?><li>
			...
			<div class="f_list_r">
				...
				<a href="<?php echo U('Product/product',array('token'=>$_GET['token'],'id'=>$hostlist['id'],'wecha_id'=>$_GET['wecha_id']));?>" class="u_order_return ts1" id="in_car"><?php if($hostlist['dining'] == 1): ?>请点餐<?php else: ?>去购买<?php endif; ?></a>
			</div>
