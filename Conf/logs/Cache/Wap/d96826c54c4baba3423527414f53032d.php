  ...
  <nav class="nav_for_list_ul">
    ...
    <div class="ofh">
      ...
      <ul>
			...
			<?php $__FOR_START_29096__=1;$__FOR_END_29096__=count($info);for($i=$__FOR_START_29096__;$i < $__FOR_END_29096__;$i+=1){ ?><li <?php if($i == 4): ?>class="on"<?php endif; ?>></li><?php } ?>
      ...
      <ul>
        ...
        <div id="nav_uls" style="width:33.3%;">
          ...
          <ol>
				...
				<?php if(is_array($cats)): $i = 0; $__LIST__ = $cats;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$hostlist): $mod = ($i % 2 );++$i;?><li><a href="javascript:swiper_2.slide(<?php echo ($i-1); ?>);" <?php if($i == 1): ?>class="on"<?php endif; ?>><img src="<?php echo ($hostlist["logourl"]); ?>" /><?php echo ($hostlist["name"]); ?></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
...
this.slide(this.length-3);
  ...
  <div id="list_uls" class="list_uls box_swipe">
	...
	<?php if(is_array($hostlist['sub'])): $i = 0; $__LIST__ = $hostlist['sub'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$res): $mod = ($i % 2 );++$i;?><dd>
				...
				<a href="<?php if($res['url'] == ''): echo U('Store/product',array('token' => $_GET['token'], 'id' => $res['id'], 'wecha_id' => $_GET['wecha_id'])); else: echo (htmlspecialchars_decode($res["url"])); endif; ?>">

            <figure>

              <div><img src="<?php echo ($res["logourl"]); ?>" /></div>
