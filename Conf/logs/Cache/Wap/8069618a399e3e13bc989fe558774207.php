...
<html><head>
		...
		<meta content="width=device-width, initial-scale=1.0, minimum-scale=1.0, maximum-scale=1.0, user-scalable=no" name="viewport">
            ...
            <meta name="apple-mobile-web-app-capable" content="yes" /> <!-- apple devices fu
			llscreen -->
            <meta name="apple-mobile-web-app-status-bar-style" content="black-translucent" />
        <style>
            img{max-width:100%!important;}
			.panel-container .container-content table {background: -webkit-linear-gradient(0deg, #eaefdc 0%, #eaefdc 50%, #eaefdc 100%);}
			.panel-container .container-content .table-bottom {background-image: -webkit-linear-gradient(0deg, #FFC45C 0%, #FFC45C 50%, #FFC45C 100%);}
			.panel-container .container-content .icon {background: url(./tpl/static/applegame/autumn/start_page_title.png);}
			.panel-container .container-content .icon_rank{background: url(./tpl/static/applegame/autumn/num.gif) no-repeat;}
			.panel-container .container-title li a.btn-activity {background: url(./tpl/static/applegame/autumn/btn-activity.png) no-repeat}
			.panel-container .container-title li a {background: url(./tpl/static/applegame/autumn/btn-rank.png) no-repeat;}
			
			.panel-container .container-content.container-rank {background: -webkit-linear-gradient(0deg, #EAF8B2 0%, #D5EE91 50%, #F9FFE7 100%);}
			#btn-start {
				background: url(./tpl/static/applegame/autumn/start_btn.png) no-repeat;
			}
        </style>
    </head>
    <body onselectstart="return true;" ondragstart="return false;">
    <style type="text/css">
        #start-page-main {background:url(./tpl/static/applegame/autumn/home_start.jpg) no-repeat center 0 #031448;background-size:100%}
    </style>
	...
	<script type="text/javascript">
        (function () {
			...
			window.start_page_config = {
                ...
                PATH:{
                    GAME:"<?php echo U('Autumn/game',array('token'=>$_GET['token'],'id'=>intval($_GET['id']),'wecha_id'=>$_GET['wecha_id']));?>"
                }
            }
					...
					<table class="table-bottom">
						...
						<col width="80%">
							...
							<?php if($_GET['wecha_id'] == ''): ?><tr>
								...
								<td colspan="2">
									请关注“<?php echo ($mpName); ?>”后回复“<?php echo ($keyword); ?>”参与此活动
								</td>
								</tr>
							...
							<tr>
							<td>
								<?php if($myinfo == NULL): ?><br /><?php else: ?>
									<span class="icon-img-bg">
										<img src="<?php echo ($myinfo["portrait"]); ?>" class="icon-img">
									</span><?php endif; ?>
							</td>
							<td align="left">
								<?php if($myinfo == NULL): ?><a style="color:#666" id="myinfo" href="<?php echo U('Userinfo/index',array('token'=>$_GET['token'],'wecha_id'=>$_GET['wecha_id'],'redirect'=>'Autumn/index|id:'.$_GET['id'].',token:'.$_GET['token'].',wecha_id:'.$_GET['wecha_id'].',type:1'));?>">您没有完善资料，请点击这里填写资料</a>
									<script>
								...
								<?php else: ?>我的排名：<b><?php echo ($myinfo["rank"]); ?></b>   最佳：<b><?php echo ($myinfo["time"]); ?></b><?php endif; ?>
...
window.shareData = {
            ...
            "timeLineLink": "<?php echo C('site_url'); echo U('Autumn/index',array('token'=>$_GET['token'],'id'=>intval($_GET['id'])));?>",
            "sendFriendLink": "<?php echo C('site_url'); echo U('Autumn/index',array('token'=>$_GET['token'],'id'=>intval($_GET['id'])));?>",
            "weiboLink": "<?php echo C('site_url'); echo U('Autumn/index',array('token'=>$_GET['token'],'id'=>intval($_GET['id'])));?>",
