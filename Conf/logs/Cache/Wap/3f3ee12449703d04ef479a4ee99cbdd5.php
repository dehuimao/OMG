...
<?php if(is_array($cats)): $i = 0; $__LIST__ = $cats;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$hostlist): $mod = ($i % 2 );++$i; if ($i > 6) {$k = $i % 6 + 1;} else {$k = $i;} ?>
	<div class="m-floor">
	<ul onclick="location.href='<?php echo U('Store/products',array('token' => $_GET['token'], 'catid' => $hostlist['id'], 'wecha_id' => $wecha_id));?>'" >
	<li class="tit bgf<?php echo ($k); ?>">
	<i><?php echo ($i); ?>F</i><?php echo ($hostlist["name"]); ?>
	</li>
	<li class="img"><a href="<?php echo U('Store/products',array('token' => $_GET['token'], 'catid' => $hostlist['id'], 'wecha_id' => $wecha_id));?>"><img src="<?php echo ($hostlist["logourl"]); ?>" style="max-width: 400px;height:180px;"/></a></li>
	</ul>
