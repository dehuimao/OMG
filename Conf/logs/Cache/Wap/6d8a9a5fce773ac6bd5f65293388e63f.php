...
<div class="body">
    ...
    <section>
        ...
        <ul class="list_ul">
            <?php if(is_array($info)): $i = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if($i%3 == 1): ?><li class="box"><?php endif; ?>
				...
				<a href="<?php if($vo['url'] == ''): echo U('Wap/Index/lists',array('classid'=>$vo['id'],'token'=>$vo['token'])); else: echo (htmlspecialchars_decode($vo["url"])); endif; ?>">
										<label><small><?php echo ($vo["name"]); ?></small></label>
						<span ><img src="<?php echo ($vo["img"]); ?>" class="icon-smile" style="width:60px;position:relative;right:-16px;"></span>
					</a>
