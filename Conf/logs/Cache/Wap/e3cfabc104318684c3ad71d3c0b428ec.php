					...
					<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><article><!--帖子-->
							...
							<section>
								...
								<?php if($list['photoArr'] != ''): ?><figure data-count="<?php echo (count($list["photoArr"])); ?>张图片">
										<div>
											<?php $__FOR_START_8586__=0;$__FOR_END_8586__=count($list['photoArr']);for($i=$__FOR_START_8586__;$i < $__FOR_END_8586__;$i+=1){ ?><img src="<?php echo ($list['photoArr'][$i]); ?>" data-src="<?php echo ($list['photoArr'][$i]); ?>" data-gid="g7" onload="preViewImg(this, event);"/><?php } ?>
										</div>
									</figure><?php endif; ?>
