...
<?php if(is_array($info)): $i = 0; $__LIST__ = array_slice($info,0,8,true);if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if($i%4 == 1): ?><ul><?php endif; ?>
...
<section>
  <nav class="nav_for_list_ul">
    <div class="ofh">
		<ul>
			<?php $__FOR_START_8738__=8;$__FOR_END_8738__=count($info);for($i=$__FOR_START_8738__;$i < $__FOR_END_8738__;$i+=1){ ?><li <?php if($i == 8): ?>class="on"<?php endif; ?>></li><?php } ?>
		...
		<ul>
			...
			<div id="nav_uls" style="width:33.3%;">
				<ol>
					<?php if(is_array($info)): $i = 0; $__LIST__ = array_slice($info,8,null,true);if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li>
							<a href="javascript:swiper_2.slide(<?php echo ($i-1); ?>);" <?php if($i == 1): ?>class="on"<?php endif; ?>>
